...
// launcher_server.py â€” deliberately a separate lightweight process/port from Core above,
// so the Control Center can still reach it (and offer to start Core) even when Core itself
// is offline.
export const LAUNCHER_BASE_URL = 'http://localhost:8050'

// Shared secret sent with every request once the backend's MAT_API_KEY is set (see
// main.py) â€” blank here matches the backend's own "auth disabled" default, so local-only
...
// literal â€” patched here once instead of touching every call site individually.
